...
import { Config, WrittenConfig } from "@abizzle/changesets-types";
...
test("read reads the config", async () => {
  ...
  expect(config).toEqual({
    ...
    linked: [],
    changelog: false,
    commit: ["@abizzle/changesets-cli/commit", { skipCI: "version" }],
    access: "restricted",
    baseBranch: "master",
    changedFilePatterns: ["**"],
...
let defaults: Config = {
  fixed: [],
  linked: [],
  changelog: ["@abizzle/changesets-cli/changelog", null],
  commit: false,
  access: "restricted",
  baseBranch: "master",
  changedFilePatterns: ["**"],
...
describe("parser errors", () => {
  test("changelog invalid value", () => {
    expect(() => {
      ...
      The \`changelog\` option is set as {} when the only valid values are undefined, false, a module path(e.g. "@abizzle/changesets-cli/changelog" or "./some-module") or a tuple with a module path and config for the changelog generator(e.g. ["@abizzle/changesets-cli/changelog", { someOption: true }])"
    `);
  });
  ...
  test("commit invalid value", () => {
    expect(() => {
      ...
      "Some errors occurred when validating the changesets config:
      The \`commit\` option is set as {} when the only valid values are undefined or a boolean or a module path (e.g. "@abizzle/changesets-cli/commit" or "./some-module") or a tuple with a module path and config for the commit message generator (e.g. ["@abizzle/changesets-cli/commit", { "skipCI": "version" }])"
    `);
  });
  describe("fixed", () => {
    test("non-array", () => {
